...
void pixel_allocator::pixel_allocator_main() {
	...
	for (int i = 0; i < 4; i++) {
		///// get data(pixel value & location) from IDP /////
		do {
			wait();
		...
		cout << endl << endl << "[Pixel allocator] " << endl;
		cout << "Location of pixel: " << "(" << IDP_input_loc_x << ", " << IDP_input_loc_y << ") = " << IDP_input_pixel << endl << endl;
